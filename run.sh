...
# Define arrays
model=('cyclegan' 'lstm' 'resnet50' 's2s' 'dqn')
outer_job_type=('training' 'training' 'training' 'training' 'training')
inner_job_type=('inference' 'inference' 'inference' 'inference' 'inference')
batch_size=(1 20 32 32 32)
...
for (( i=0; i<${#model[@]}; i++ )); do
    for (( j=0; j<${#model[@]}; j++ )); do
        ...
        python3 parse_log.py -l "models/${model[i]}/${outer_log_file[i]},models/${model[j]}/${inner_log_file[j]}" --output "outer_${model[i]}_${outer_job_type[i]}_inner_${model[j]}_${inner_job_type[j]}.json"
    done
done
